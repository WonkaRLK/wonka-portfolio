...
const services = [
  {
    ...
    icon: (
      <svg className="w-10 h-10 text-wonka-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <section id={id} className="pt-36 pb-20 px-6 max-w-5xl mx-auto">
      <ScrollReveal>
        <h2 className="font-heading font-bold text-4xl sm:text-5xl text-wonka-gold text-glow-gold text-center mb-14">
          <EncryptedText
            text="Qué hago"
            encryptedClassName="text-wonka-gold/40"
            revealedClassName="text-wonka-gold"
            revealDelayMs={80}
            animateOn="view"
          />
        </h2>
          ...
          <ScrollReveal key={service.title} delay={i * 0.15}>
            <CardContainer containerClassName="w-full" className="w-full">
              <CardBody className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-2xl p-8 text-center w-full hover:shadow-[0_20px_60px_rgba(212,168,67,0.35)] transition-shadow duration-300">
                ...
                <CardItem translateZ={80} as="p" className="font-body text-sm text-wonka-cream-dark leading-relaxed w-full">
                  {service.description}
                </CardItem>
              </CardBody>
            </CardContainer>
